...
if ! $(grep -q "^source '$HOME/.bashrc.common'" "$HOME/.bashrc" > /dev/null); then
  echo "source '$HOME/.bashrc.common'" >> "$HOME/.bashrc"
fi

# append OS-specific functions, aliases, etc to bashrc
if [[ "$OSTYPE" == "linux-gnu" ]]; then
  if ! $(grep -q "^source '$HOME/.bashrc.linux'" "$HOME/.bashrc" > /dev/null); then
    echo "source '$HOME/.bashrc.linux'" >> "$HOME/.bashrc"
  fi
elif [[ "$OSTYPE" == "darwin"* ]]; then
  if ! $(grep -q "^source '$HOME/.bashrc.macos'" "$HOME/.bashrc" > /dev/null); then
